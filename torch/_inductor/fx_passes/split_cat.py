...
def merge_getitem_stack(match: Match, unbind_input: torch.fx.Node, dim: int):
    ...
    for stack_user in next_users:
        if stack_user.target == torch.stack:
            ...
            if (
                unbind_dim != stack_dim
                or not has_same_parent_node(stack_user)
            ):
                continue
            ...
            for getitem in stack_user.args[0]:  # type: ignore[union-attr]
                ...
                idx_to_getitem[getitem.args[1]] = getitem  # type: ignore[index]
            ...
            if len(unbind_node.users) == len(stack_user.args[0]):  # type: ignore[union-attr]
